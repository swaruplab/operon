...
Operon is a desktop IDE for researchers, built with Tauri 2 (Rust backend + React/TypeScript frontend). It wraps Codex into a professional multi-panel developer environment with an integrated terminal, file browser, Monaco code editor, and AI chat interface. Designed for scientists who work on HPC compute clusters — it runs Codex agents directly on remote compute nodes via SSH and tmux, with sessions that persist across app restarts.
...
| AI | Codex headless mode (stream-json) | Structured NDJSON streaming |
...
│   │       ├── Codex.rs               # ClaudeManager + session management + dependency checking + setup wizard backend
...
│   │   ├── Codex.ts                    # Typed wrappers: checkInstalled, auth, startSession, etc.
...
│       │
│       ├── chat/
│       │   └── ChatPanel.tsx             # Codex chat: streaming, thinking blocks, tool display, session resume, plan detection
│       │
│       ├── settings/
│       │   └── SettingsPanel.tsx          # Modal: editor/terminal/Codex/auth settings
│       │
│       └── setup/
│           └── SetupWizard.tsx           # First-time setup: dependency checks, Codex install, API key
│
├── package.json
├── vite.config.ts
├── tsconfig.json
...
8. **Codex**: `Codex -p --verbose --output-format stream-json` gives NDJSON. Parsed line-by-line.
...
11. **Terminal Mode**: Commands must run in the terminal's own shell (not piped to `bash`) to preserve user aliases like `Codex → npx @anthropic-ai/Codex`.
...
Terminal mode is the primary execution mode for HPC users. It runs Codex agents inside existing tmux sessions on compute nodes.
...
- **On session start**: Metadata saved to `~/.operon/sessions/{session_id}.json` (Codex CLI session ID, project path, SSH profile, mode, timestamps)
- **During streaming**: Codex CLI session ID captured from `system` event and persisted for `--resume`
...
2. **Dependency check**: Xcode CLI tools, Codex
3. **Installation**: Individual or "Install All Missing" (Xcode via `xcode-select --install`, Codex via `curl -fsSL https://Codex.ai/install.sh | bash`, falls back to npm if curl fails)
...
Codex's `--output-format stream-json` sends cumulative content for each assistant message. Each `assistant` event contains the FULL content for that message so far.
...
- **Phase 5** ✅ Codex Integration — Detection & installation, API key auth, headless stream-json NDJSON parsing, chat UI with tool use display, model selector, cost tracking, session resume
...
- **Phase 7** ✅ Polish — Settings panel (editor/terminal/Codex/auth), JSON config persistence, Cmd+comma shortcut, error handling
- **Phase 8** ✅ HPC Terminal Mode — Run Codex agents inside existing tmux/compute node sessions, output on shared filesystem, separate SSH tail for streaming, alias-aware command injection
...
- **`Codex` is an alias on HPC**: Resolves to `npx @anthropic-ai/Codex`. Terminal commands must run in the user's shell to access the alias. Never pipe through `| bash` or the alias is lost.
...
- **`--output-format stream-json` requires `--verbose`**: Without it, Codex exits with an error.
