        ...
        {ordered.map((e, i) => {
          ...
          return (
            <div key={`${e.ts}-${e.script}-${i}`}>
              <button
                onClick={() => blockedOne && setExpanded(isOpen ? null : i)}
                className={`w-full flex items-center gap-1.5 px-2 py-1 rounded text-[11px] text-left ${
                  blockedOne ? 'hover:bg-hover cursor-pointer' : 'cursor-default'
                }`}
              >
                {e.outcome === 'clean' ? (
              ...
              {blockedOne && isOpen && (
                <div className="ml-6 mr-2 mb-1 space-y-1">
                  {e.findings.map((f, j) => (
                    <div
                      key={j}
                      className="rounded border border-red-600/40 bg-red-600/5 px-2 py-1 text-[10px]"
                    >
