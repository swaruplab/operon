...
For long-running HPC batch jobs, Operon deploys a lightweight watchdog agent as a bash script inside a dedicated tmux session on the remote host. The watchdog polls the job scheduler (SLURM, with stubs for PBS, LSF, and SGE) at configurable intervals, appends NDJSON events to per-job log files under `~/.operon/jobs/`, and applies user-defined policies for automatic job resubmission on timeout or out-of-memory failures, up to a configurable retry budget. Because the watchdog runs in tmux, it persists through Operon application restarts and SSH disconnections. Operon streams watchdog events back to the frontend using the same SSH tail mechanism as the AI chat, providing real-time job status updates in the sidebar.
